...
import { execFile, sleep } from "@onslip/automation/test";
import {
  DeviceWorkerLauncher,
  DeviceWorkerLauncherOptions,
} from "@onslip/automation/test";
...
export class AndroidLauncher extends DeviceWorkerLauncher<AndroidLauncherOptions> {
  ...
  override async setup(
    ...
    if (this.options?.emulator) {
      ...
      dwConfig?.console?.info?.(
        `⚙️  Creating AVD ${this.options.emulator.name(worker)}`,
      );
      await execFile("avdmanager", [
        "--silent",
        "create",
        "avd",
        "--force",
      ...
      dwConfig?.console?.info?.(
        `⚙️  Starting ${headless ? "headless " : ""}emulator ${this.options.emulator.name(worker)} as device ${deviceId}`,
      );
      ...
      for (
        ...
        i % 10 ||
          dwConfig?.console?.debug?.(
            `🐢 Waiting for device ${deviceId} to boot ...`,
          );
        await sleep(1000);
      }
      ...
      for (const port of this.options.emulator.ports ?? []) {
        dwConfig?.console?.info?.(
          `⚙️  Forwarding localhost port ${port} from device ${deviceId} to host`,
        );
        await execFile("adb", [
          "-s",
          deviceId,
  ...
  override async teardown(worker: number, workerInfo: WorkerInfo) {
    ...
    if (this.options?.emulator) {
      ...
      this.emulators[worker]?.child.kill();
      await this.emulators[worker]?.catch(
        dwConfig?.console?.error ?? console.error,
      );
